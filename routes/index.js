...
router.post('/register', async (req, res) => {
    ...
    try {
        await registerApplication(app_id, app_name);
        res.status(200).json({ success: true, message: `Application ${app_name} registered successfully` });
    } catch (error) {
        res.status(500).json({ error: 'Failed to register application', details: error.message });
    }
});
...
router.post('/send', async (req, res) => {
    ...
    try {
        const appName = await getApplicationName(app_id);
        if (!appName) {
